...
require_once('model_gestiones.php');
require_once('../core/render_view_generic.php');
class controller_gestiones extends gestiones{
	
	protected $diccionario = array(
		'subtitle'=>array('agregar'=>'Crear Gestiones',
                      'buscar'=>'Buscar Gestiones',
                      'borrar'=>'Eliminar  Gestiones',
                      'modificar'=>'Modificar Gestiones',
                      'listar'=>'Lista  Gestiones'),
		'links_menu'=>array(
						'VIEW_SET_USER'=>'ventas/controller_gestiones.php?act=set',
						'VIEW_GET_USER'=>'ventas/controller_gestiones.php?act=buscar',
						'VIEW_EDIT_USER'=>'ventas/controller_gestiones.php?act=modificar',
						'VIEW_DELETE_USER'=>'ventas/controller_gestiones.php?act=borrar'),
		'form_actions'=>array(
							'SET'=>'../ventas/controller_gestiones.php?act=insert',
							'GET'=>'../ventas/controller_gestiones.php',
        'DELETE'=>'../ventas/controller_gestiones.php?act=delete',
        'EDIT'=>'../ventas/controller_gestiones.php?act=edit',
        'GET_ALL'=>'../ventas/controller_gestiones.php?act=get_all'
		)
	);
...
//	protected $msg;
	...
	public function helper_parametros_data() {
		...
		if(isset($_REQUEST)) {
			...
			if(array_key_exists('ID_CONTACTO_X_EMPRESA', $_REQUEST)) { 
				$parametros_data['ID_CONTACTO_X_EMPRESA'] = $_REQUEST['ID_CONTACTO_X_EMPRESA']; 
			}
			if(array_key_exists('ID_TIPO_GESTION', $_REQUEST)) { 
				$parametros_data['ID_TIPO_GESTION'] = $_REQUEST['ID_TIPO_GESTION']; 
			}
			if(array_key_exists('FECHA', $_REQUEST)) { 
				$parametros_data['FECHA'] = $_REQUEST['FECHA']; 
			}
			if(array_key_exists('ID_RECORDATORIO', $_REQUEST)) { 
				$parametros_data['ID_RECORDATORIO'] = $_REQUEST['ID_RECORDATORIO']; 
			}			
			if(array_key_exists('COD_CIA', $_REQUEST)) { 
				$parametros_data['COD_CIA'] = $_REQUEST['COD_CIA']; 
			}
			if(array_key_exists('COD_EMP', $_REQUEST)) { 
				$parametros_data['COD_EMP'] = $_REQUEST['COD_EMP']; 
			}
			if(array_key_exists('ESTILO', $_REQUEST)) { 
				$parametros_data['ESTILO'] = $_REQUEST['ESTILO']; 
			}		
			if(array_key_exists('PRECIO', $_REQUEST)) { 
				$parametros_data['PRECIO'] = $_REQUEST['PRECIO']; 
			}										
			if(array_key_exists('CANTIDAD', $_REQUEST)) { 
				$parametros_data['CANTIDAD'] = $_REQUEST['CANTIDAD']; 
			}
			if(array_key_exists('OBSERVACIONES', $_REQUEST)) { 
				$parametros_data['OBSERVACIONES'] = $_REQUEST['OBSERVACIONES']; 
			}							
		} else if($_REQUEST) {
			if(array_key_exists('ID', $_REQUEST)) {
	...
	public function delete(){
		...
		$parametros = $this->set_obj();
		$obvista = new view_Parametros();
		$parametros->delete('gestiones',$parametros_data);
	...
	public function get_all(){
		...
		$masx= array($parametros->tableName().'.ID',
					 "contactos_x_empresas.NOMBRES||'|'||contactos_x_empresas.ID ",
					 "tipos_gestiones.NOMBRE||'|'||tipos_gestiones.ID",
					 $parametros->tableName().'.ID_RECORDATORIO');
        			 $masx=implode($masx, ",");
		$links =array("","controller_contactos_x_empresas","controller_tipo_gestion","controller_recordatorios");         			 
...
$objecon =  new controller_gestiones();
$objecon->handler();
